...
ACR:            acr-eia-<environment>-<suffix>
...
The script publishes the .NET application, builds a `linux/amd64` image with `Dockerfile.runtime`, pushes it to ACR, deploys the Bicep infrastructure, configures Entra authentication, assigns managed-identity permissions, updates the Container App, and runs authenticated health and Cosmos DB smoke tests.
